...
class PriceResourceTest {
  ...
  void testGetTomorrowsEntries() {
    Mockito.when( priceRepository.getPrices(Mockito.any(),Mockito.any())).thenReturn(makeEntriesWithTomorrow());
    Mockito.when(dateService.now()).thenReturn(OffsetDateTime.of(2025, 3, 29, 13,1,1,1, DateUtils.HELSINKI_ZONE.getRules().getOffset(Instant.now())));
  ...
  void testGetTomorrowsEntriesMissingAtOne() {
    Mockito.when( priceRepository.getPrices(Mockito.any(),Mockito.any())).thenReturn(makeEntriesWithoutAll());
    Mockito.when(dateService.now()).thenReturn(OffsetDateTime.of(2025, 3, 29, 1,1,1,1, DateUtils.HELSINKI_ZONE.getRules().getOffset(Instant.now())));
    ...
    given()
            ...
            .then()
            .statusCode(200)
            .header(EXPIRES_HEADER,  allOf(startsWith("Sat, 29 Mar 2025 11:57:"), endsWith("GMT")))
  ...
  void testGetTomorrowsEntriesMissing() {
    Mockito.when( priceRepository.getPrices(Mockito.any(),Mockito.any())).thenReturn(makeEntriesWithoutAll());
    Mockito.when(dateService.now()).thenReturn(OffsetDateTime.of(2025, 3, 29, 10,1,1,1, DateUtils.HELSINKI_ZONE.getRules().getOffset(Instant.now())));
  ...
  void testGetTomorrowsEntriesMissingEarlier() {
    Mockito.when( priceRepository.getPrices(Mockito.any(),Mockito.any())).thenReturn(makeEntriesWithoutAll());
    Mockito.when(dateService.now()).thenReturn(OffsetDateTime.of(2025, 3, 29, 13,58,1,1, DateUtils.HELSINKI_ZONE.getRules().getOffset(Instant.now())));
  ...
  private static PriceRepository.PriceHistoryEntry makeEntry(final String end, final String start, final double price) {
    return new PriceRepository.PriceHistoryEntry(BigDecimal.valueOf(price), OffsetDateTime.parse(start), OffsetDateTime.parse(end));
  }
  ...
  private static List<PriceRepository.PriceHistoryEntry> makeEntriesWithoutAll() {
    return List.of(
  ...
  private static List<PriceRepository.PriceHistoryEntry> makeEntriesWithTomorrow() {
    return List.of(
            makeEntry("2025-03-27T23:00:00Z", "2025-03-27T22:00:00Z", 3.4 ),
            makeEntry("2025-03-28T00:00:00Z", "2025-03-27T23:00:00Z", 2.99 ),
            makeEntry("2025-03-28T01:00:00Z", "2025-03-28T00:00:00Z", 2.23 ),
